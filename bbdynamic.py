...
example_graph.heuristic_for_each_node ={    
...
# A* searcj function
def bb_dynamic_search(graph, start, goal): # here the goal must be 'M', since we already stored the heuristic costs
    ...
    cost_so_far = {}       # cumulate the real path cost to current node so far 
    ...
    while not frontier.empty():
        ...
        for next in graph.neighbors(current):
            ...
            if next not in cost_so_far or new_cost < cost_so_far[next]: #new_cost < cost_so_far[next] is the dynamic programming condition
                ...
                priority = new_cost   # real path cost to "NEXT" node so far
...
reconstruct_path(bb_dynamic_search(example_graph, 'A','M'),'A','M')
